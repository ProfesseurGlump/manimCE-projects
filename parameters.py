...
class Search(Scene):
    def construct(self):
        ...
        pointer_length = 0.4
        l_pointer = Arrow(
            start = DOWN * pointer_length,
            end = UP
        ).next_to(squares[0], DOWN)
        r_pointer = Arrow(
            start = DOWN * pointer_length,
            end = UP
        ).next_to(squares[-1], DOWN)

        self.play(
            *[Write(s) for s in squares],
            *[Write(n) for n in numbers]
        )

        # print the number we're looking for
        nb_value = value_max - value_min + 1
        if nb_value % 2 == 1:
            median_index = (nb_value - 1) // 2
            target = values[median_index]
        else:
            median_index_inf = nb_value / 2
            median_index_sup = nb_value / 2 + 1
            med_inf = values[int(median_index_inf)]
            med_sup = values[int(median_index_sup)]
            target = (med_inf + med_sup) / 2
        text = Tex(f"Médiane : ${target}$").shift(UP * 1.5)
        self.play(Write(text))

        lo, hi = 0, len(values) - 1

        def color_in_range(objects, color, range):
            """Return the animation of coloring
        ...
        self.play(
            *[FadeOut(r) for r in numbers + squares + [r_pointer, text]]
        )
